...
#define MAX_FRAMEBUFFERS 1
static unsigned char * framebuffers[MAX_FRAMEBUFFERS] = {0};
// REMI BERTHOLET END
...
// REMI BERTHOLET START
        if (x < MAX_FRAMEBUFFERS)
        {
            if (framebuffers[x] == 0)
            {
                framebuffers[x] = (uint8_t *)heap_caps_aligned_alloc(16, alloc_size, _caps);
            }
            cam_obj->frames[x].fb.buf = framebuffers[x];
        }
        else
// REMI BERTHOLET
