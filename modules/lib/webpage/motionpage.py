...
async def motion(request, response, args):
	...
	page = mainFrame(request, response, args,b"Motion detection configuration",
		...
		Slider(text=b"Detect motion when the contigous differences on two successive images is greater or equal to",          name=b"contigousDetection",        min=b"1",  max=b"50", step=b"1",  value=b"%d"%config.contigousDetection,         disabled=disabled),
