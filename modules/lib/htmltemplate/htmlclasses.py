...
# </div>
begTagSwitch = b'''<div class="custom-control custom-switch"><input type="hidden" value="0" name="%s" /><input type="checkbox" class="custom-control-input %s" style="%s" id="%s" value="%s" name="%s" %s %s onchange="%s" /><label class="custom-control-label" for="%s">%s</label></div>'''
def Switch(*args, **params):
	self = Template(*(("Switch",) + args), **params)

	def getBegin(self):
		global begTagSwitch
		return begTagSwitch%(self.name,self.class_,self.style,self.id,self.value,self.name, b'checked' if self.checked else b'', b'disabled' if self.disabled else b'',self.onchange,self.id,self.text)
	self.getBegin     = getBegin
...
# </label>
# </div>
begTagCheckbox = b'''<div class="form-check"><label class="form-check-label"><input type="checkbox" class="form-check-input %s" style="%s" id="%s" value="%s" name="%s" %s %s>%s</input></label></div>'''
...
begTagButton = b'''<button class="btn btn-outline-primary %s" style="%s" id="%s" type="%s" name="%s" value="%s" %s>%s</button>'''
def Button(*args, **params):
	self = Template(*(("Button",) + args), **params)

	def getBegin(self):
		global begTagButton
		return begTagButton%(self.class_,self.style,self.id,self.type,self.name,self.value, b'disabled' if self.disabled else b'',self.text)
	self.getBegin     = getBegin
...
# </label>
# </div>
begTagRadio = b'''<div class="form-check"><label class="form-check-label"><input type="radio" class="form-check-input %s" style="%s" id="%s" name="%s" %s %s onchange="%s">%s</input></label></div>'''
def Radio(*args, **params):
	self = Template(*(("Radio",) + args), **params)

	def getBegin(self):
		global begTagRadio
		return begTagRadio%(self.class_,self.style,self.id,self.name, b'checked' if self.checked else b'', b'disabled' if self.disabled else b'',self.onchange,self.text)
	self.getBegin     = getBegin
...
# %(content)s
# </form>
begTagForm = b'''<form class="container %s" style="%s" id="%s" method="%s" action="%s">'''
endTagForm = b'''</form>'''
def Form(*args, **params):
	...
	def getBegin(self):
		global begTagForm
		return begTagForm%(self.class_,self.style,self.id,self.method,self.action)
	self.getBegin     = getBegin
...
begTagCardHeader = b'''<div class="card-header %s" style="%s" id="%s">%s '''
endTagCardHeader = b'''</div>'''
def CardHeader(*args, **params):
	self = Template(*(("CardHeader",) + args), **params)

	def getBegin(self):
		global begTagCardHeader
		return begTagCardHeader%(self.class_,self.style,self.id,self.text)
	self.getBegin     = getBegin

	def getEnd(self):
		global endTagCardHeader
		return endTagCardHeader
	self.getEnd       = getEnd
...
begTagCardBody = b'''<div class="card-body %s" style="%s" id="%s">'''
endTagCardBody = b'''</div>'''
def CardBody(*args, **params):
	self = Template(*(("CardBody",) + args), **params)

	def getBegin(self):
		global begTagCardBody
		return begTagCardBody%(self.class_,self.style,self.id)
	self.getBegin     = getBegin

	def getEnd(self):
		global endTagCardBody
		return endTagCardBody
	self.getEnd       = getEnd
...
# %(content)s
# </div>
begTagCard = b'''<div class="card %s" style="%s" id="%s">'''
endTagCard = b'''</div>'''
...
begTagSubmit = b'''<button class="btn btn-outline-primary %s" style="%s" id="%s" type="submit" name="%s" value="%s" onclick="%s">'''
...
def Submit(*args, **params):
	...
	def getBegin(self):
		global begTagSubmit
		return begTagSubmit%(self.class_,self.style,self.id,self.name,self.value,self.onclick)
	self.getBegin     = getBegin
...
begTagMenuItem = b'''<a class="dropdown-item %s %s" style="%s margin-right: 2px;" id="%s" href="%s" %s>%s</a>'''
def MenuItem(*args, **params):
	self = Template(*(("MenuItem",) + args), **params)

	def getBegin(self):
		global begTagMenuItem
		return begTagMenuItem%( b'active' if self.active else b'',self.class_,self.style,self.id,self.href, b'disabled' if self.disabled else b'',self.text)
	self.getBegin     = getBegin
...
# </div>
# </li>
begTagMenu = b'''<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" %s>%s</a><div class="dropdown-menu">'''
endTagMenu = b'''</div></li>'''
def Menu(*args, **params):
	...
	def getBegin(self):
		global begTagMenu
		return begTagMenu%( b'disabled' if self.disabled else b'',self.text)
	self.getBegin     = getBegin
...
# </ul>
# </nav>
begTagMenuBar = b'''<nav class="navbar navbar-expand-sm bg-light navbar-light" style="background-color: #e3f2fd;"><ul class="navbar-nav" style="%s" id="%s">'''
endTagMenuBar = b'''</ul></nav>'''
def MenuBar(*args, **params):
	self = Template(*(("MenuBar",) + args), **params)

	def getBegin(self):
		global begTagMenuBar
		return begTagMenuBar%(self.style,self.id)
	self.getBegin     = getBegin

	def getEnd(self):
		global endTagMenuBar
		return endTagMenuBar
	self.getEnd       = getEnd
...
# <a class="dropdown-item %(active)s %(class_)s" style="%(style)s margin-right: 2px;" id="%(id)s" href="%(href)s" %(disabled)s>%(text)s</a>
begTagMenuItem_ = b'''<a class="dropdown-item %s %s" style="%s margin-right: 2px;" id="%s" href="%s" %s>%s</a>'''
def MenuItem_(*args, **params):
	self = Template(*(("MenuItem_",) + args), **params)

	def getBegin(self):
		global begTagMenuItem_
		return begTagMenuItem_%( b'active' if self.active else b'',self.class_,self.style,self.id,self.href, b'disabled' if self.disabled else b'',self.text)
	self.getBegin     = getBegin
...
# </div>
# </div>
begTagMenu_ = b'''<div class="dropdown %s %s" style="%s" id="%s" ><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" %s>%s</button><div class="dropdown-menu">'''
endTagMenu_ = b'''</div></div>'''
def Menu_(*args, **params):
	self = Template(*(("Menu_",) + args), **params)

	def getBegin(self):
		global begTagMenu_
		return begTagMenu_%( b'active' if self.active else b'',self.class_,self.style,self.id, b'disabled' if self.disabled else b'',self.text)
	self.getBegin     = getBegin
...
# %(content)s
# </div>
begTagMenuBar_ = b'''<div class="btn-group%s" style="%s" id="%s">'''
endTagMenuBar_ = b'''</div>'''
def MenuBar_(*args, **params):
	self = Template(*(("MenuBar_",) + args), **params)

	def getBegin(self):
		global begTagMenuBar_
		return begTagMenuBar_%(self.class_,self.style,self.id)
	self.getBegin     = getBegin

	def getEnd(self):
		global endTagMenuBar_
		return endTagMenuBar_
	self.getEnd       = getEnd
...
endTagComboBox = b'''</datalist></input><br>'''
...
# </div>
# </div>
begTagModal = b'''<div class="modal-dialog modal-dialog-centered %s" style="align-items:center;min-height:100vh; %s" id="%s"><div class="modal-content shadow-lg p-3 mb-5 bg-white rounded"><div class="modal-body" style="padding:10px 10px;">'''
endTagModal = b'''</div></div></div>'''
def Modal(*args, **params):
	self = Template(*(("Modal",) + args), **params)

	def getBegin(self):
		global begTagModal
		return begTagModal%(self.class_,self.style,self.id)
	self.getBegin     = getBegin
